...
exports.login = (name, password) => {
    var user_id = db.prepare('SELECT id FROM user WHERE name=? AND password=?')[name, password];
    if (user_id == null) {
        return -1;
    } else return user_id;
}
...
exports.printProfil = (id) => {
    var user_name = db.prepare('SELECT name FROM user WHERE id=?')[id];
    return user_name;
}